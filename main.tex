...
而是通过反复迭代从输入数据集中选出“典范（exemplars）”来代表一个簇。
...
首先责任矩阵$r(i,k)$量化了第$k$个元素适合成为第$i$个元素的典范的程度，并考虑了最近的竞争者第$k^{'}$个元素可能成为第$i$个元素的典范这个因素。
\begin{equation}\label{eqn:resp_matrix}
    r(i, k) \leftarrow s(i, k)-\max _{k^{\prime} \neq k}\left\{a\left(i, 
k^{\prime}\right)+s\left(i, k^{\prime}\right)\right\}
\end{equation}

接着获取度矩阵的非对角线元素$a(i,k)$量化了第$i$个元素选择第$k$个元素作为典范的合适程度，考虑了其他元素支持k成为一个典范的程度。
...
获取度矩阵的对角线元素$a(k,k)$反映了第$k$个元素适合成为一个典范的累积证据，基于k对于其他元素的积极责任。
...
\begin{figure}
   ...
   \end{subfigure}
   \caption{\small 二个二次采样的数据集 1.1.n and 1.2.n 在基础数据集上的分布}
\end{figure}
