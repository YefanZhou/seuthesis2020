...
也有少量基于学习的方法如Make3D\cite{10.1109/TPAMI.2008.132}使用了马尔可夫随机场（Markov Random Field）来将单目线索与图片各部分的关系进行建模，能够重建基础
简单的几何形状。文章\cite{7298807}的研究者采用了非刚性从运动恢复结构模型（NRSfM）并结合现有的目标检测数据集中的二维标注来重建三维模型。
尽管这些工作在三维重建领域做出了从无到有的伟大进展，但是这些方法依然不能强鲁棒性的从单张图片重建完整和高水平的三维形状，他们的方法依然缺少
较强的形状先验。
...
令人惊讶的是，同年的CVPR，还有一篇文章AltasNet\cite{groueix2018papier}，提出了从单张图片重建三维网格的深度神经网络模型，
且其核心部分解码器架构与FoldingNet\cite{yang2018foldingnet}解码器的架构思想十分相似，且该模型至今仍保持了当前单视角三维重建任务的最优
...
一个神经网络模型从输入图片$l$预测以点云表示的形状$\widehat{S}$，在训练中通过减小特定的损失函数定义的经验损失$l$，使得$\widehat{S}$的形状接近标签点云$S$。该过程的数学定义为:
\begin{equation}
...
在这个部分，我们定义用来测量数据集聚类趋势的指标。具体来说，我们使用silhouette score\cite{scikit-learn}来测量聚类趋势。
给定一个数据集$D = \{x_i\}_{i=0}^{N-1}$和一个随机距离$d(x,y)$方程\footnote{我们考虑满足下面三个属性的任何距离方程，
...
\begin{itemize}
    ...
    \item \textit{Oracle-Nearest-Neighbor\footnote{下文缩写为Oracle-NN}}：该模型在预测时，在训练集中直接搜索与标签点云损失最小的训练点云，将其
...
\end{figure}
根据论文PSGN\cite{fan2017point}提出的基于自编码器架构\footnote{实际为“编码器-解码器”架构，因为输入输出分别是同一个物体的图片与三维点云形状所以简称为自编码器架构}的深度神经网络模型，我们进行了如图\ref{fig:ae_baseline}的实现。网络的主要架构如下：
\begin{itemize}
\item 编码器：采用ResNet-18\cite{7780459}的卷积层架构作为接受二维图像输入并提取图像特征的编码器, 具体来说，移除ResNet-18最后一层用来分类的全连接层，将一维特征层直接输出作为码字。
...
擅长拟合非线性函数，因此它们可以在二维网格上执行精细的折叠操作。这一折叠操作的可视化展示在图\ref{fig:folding_demo}。本文使用的解码器架构包含有两个感知器模块，第一个模块将二维网格折叠到三维空间，
第二个模块在三维空间内继续折叠网格，使其接近标签形状。

图\ref{fig:training_process_vis}展示了随着训练周期的增加，这一深度神经网络基准模型逐渐学会折叠操作的过程。
...
\begin{figure}[ht!]
    \begin{centering}
        \begin{tabular}{ccccc}
        \toprule
         输入图片  & 二维网格点 & 第一次折叠 & 第二次折叠   \\
        \toprule
        \includegraphics[width=0.23\columnwidth,keepaspectratio]{figs/folding_demo/image_019_num_177.png} &
        &
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/prim_019_num_177.png} &  
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/recon_019_num_177.png} \\
        %& DI = 1 & & & DI = 1 & \\
        %\midrule
        \includegraphics[width=0.23\columnwidth,keepaspectratio]{figs/folding_demo/image_040_num_21.png} &
        \includegraphics[width=0.15\columnwidth,keepaspectratio]{figs/folding_demo/2D_grid.png} &
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/prim_040_num_21.png} &  
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/recon_040_num_21.png} \\
        %& DI = 2 & & & DI = 2 & \\
        %\midrule
        \includegraphics[width=0.23\columnwidth,keepaspectratio]{figs/folding_demo/image_025_num_84.png} &
         &
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/prim_025_num_84.png} &  
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/recon_025_num_84.png} \\
        %& DI = 3 & & & DI = 3 & \\
        %\midrule
        \includegraphics[width=0.23\columnwidth,keepaspectratio]{figs/folding_demo/image_019_num_144.png} &
         &
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/prim_019_num_144.png} &  
        \includegraphics[width=0.28\columnwidth,keepaspectratio]{figs/folding_demo/recon_019_num_144.png} \\
        \bottomrule  
        \end{tabular}  \vspace{-2mm}
        \caption{\small 两步折叠解码的展示}
        \label{fig:folding_demo}
    \end{centering}
    \end{figure}
...
\begin{figure}
    ...
    \begin{tabular}{cccccccc}
        \toprule
        输入图片    & 周期5  & 周期15&  周期25& 周期35 & 周期45&  周期80 & 周期100 \\
        \toprule
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_image_005.png} &
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_005.png} &
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_015.png} &
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_025.png} &
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_045.png} &
		\includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_055.png} &
        \includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_200.png} &
        \includegraphics[width=.11\textwidth,keepaspectratio]{figs/baseline_vis/cube_recon_300.png} \\
        \bottomrule
    \end{tabular}
	\caption{\small 训练过程中测试结果的展示。训练中的神经网络逐渐学会将随机的二维平面折叠为三维形状的表面。}
    \label{fig:training_process_vis} \vspace{-4mm}
\end{centering}
\end{figure}
\FloatBarrier
...
对于基于自编码器架构的深度学习基准模型，我们以Chamfer Distance\footnote{Chamfer Distance数值越小说明重建效果越好}作为损失函数，使用Adam优化器，训练了100个周期，使其达到收敛，并对超参数进行调参，总共进行了8次训练，最终展示了
...
结合图\ref{fig:lossplot}展示的在不同学习率下深度神经网络的两次训练过程的损失变化\footnote{其他所有超参数如权重损失，动量等均相同}, 对这些问题的分析如下：
\begin{itemize}
...
如图\ref{fig:optimal_lossplot}展示了基准模型与优化后的模型的训练过程，三次训练的初始学习率相同\footnote{基准模型训练过程学习率衰减为第30，60，90周期降低10倍，
优化模型学习率衰减为第40，80周期降低10倍，其余超参数相同。}。可以看出两种方法优化后的模型（蓝色线，橙色线）展示出了稳定的训练过程，训练损失收敛正常且没有出现梯度消失导致损失停止更新
的问题（如绿色线）。表\ref{table:metric_optimal_baseline}汇报了基准模型，两种优化模型以及Oracle-NN的最优平均重建损失（Chamfer Distance），
...
\begin{figure}[!htb]
    \centering
       \begin{subfigure}[t]{0.49\linewidth} \centering 
         \includegraphics[width=\columnwidth]{figs/model/ae_optimal_trainloss.png}
         \caption{训练集损失}\label{fig:trainingloss}
       \end{subfigure}
      \begin{subfigure}[t]{0.49\linewidth} \centering
          \includegraphics[width=\columnwidth]{figs//model/ae_optimal_testloss.png}
          \caption{测试集损失}\label{fig:testloss}
      \end{subfigure}
       \caption{\small 深度神经网络基准（绿色）/优化模型（蓝色和橙色）的训练过程。三次神经网络训练的初始学习率相同。红色线指非深度学习基准模型Oracle-NN的量化结果}
    \label{fig:optimal_lossplot}
\end{figure}
...
从二次采样数据集2.1.1到2.1.6，聚集趋势逐渐变低。如图\ref{fig:subsample-dataset-2.1}
...
\begin{figure}[ht!]
    \begin{centering}
        ...
        \toprule
        数据集2.1.1 & 数据集2.1.2 & 数据集2.1.3  \\
        \toprule
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_0.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_1.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_2.png} \\
        %& DI = 1 & & & DI = 1 & \\
        \midrule
        数据集2.1.4 & 数据集2.1.5 & 数据集2.1.6  \\
        \midrule
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_3.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_4.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_random_5.png} \\
        \bottomrule  
        \end{tabular}  \vspace{-2mm}
        \caption{\small 二次采样数据集2.1。每个圆点代表一个样本对（图像，点云）。蓝色的点代表一个以标准球形状为中心的簇采样，
        绿色点代表以标准正四面体为中心的簇采样，橙色点代表以标准正立方体为中心的簇采样。}
        \label{fig:subsample-dataset-2.1}
    \end{centering}
\end{figure}
\FloatBarrier

\textbf{二次采样数据集2.2 描述章节\ref{sec:case1}的情况1和情况3} ~二次采样数据集2.2的目的是覆盖从情况3到情况1的变化。它包含了六个数据集
2.2.n（n=1,2,$\ldots$6）。从二次采样数据集2.2.1到2.2.6，训练图片集的聚类趋势逐渐降低，同时训练点云集的聚类趋势保持在高水平。如图\ref{fig:subsample-dataset-2.2}
...
\begin{figure}[ht!]
    \begin{centering}
        \begin{tabular}{cccc|cccc}
        \toprule
        数据集2.2.1 & 数据集2.2.2 & 数据集2.2.3  \\
        \toprule
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_0.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_1.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_2.png} \\
        %& DI = 1 & & & DI = 1 & \\
        \midrule
        数据集2.2.4 & 数据集2.2.5 & 数据集2.2.6  \\
        \midrule
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_3.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_4.png} &
        \includegraphics[width=0.33\columnwidth,keepaspectratio]{figs/three-clusters/sample_5.png} \\
        \bottomrule  
        \end{tabular}  \vspace{-2mm}
        \caption{\small 二次采样数据集2.2。每个圆点仅代表一个样本对中的图像，且其对应的点云由所属簇中的叉所代表。
                    如蓝色圆点所对应的点云均为坐标（0.0,0.5)处的叉。蓝色的点代表一个以标准球形状为中心的图像簇采样，
                    绿色点代表以标准正四面体为中心的图像簇采样，橙色点代表以标准正立方体为中心的图像簇采样。
        }
        \label{fig:subsample-dataset-2.2}
    \end{centering}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.95\textwidth]{figs/silhouette-dataset-1_roman.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.95\textwidth]{figs/silhouette-dataset-2_roman.png}
...
\textbf{本课题具有如下创新点：}
\begin{itemize}
    \item[1.] 提出了单视角三维重建任务中行之有效的深度神经网络优化方案。提出两种创新的优化方法，一是使用残差层改进基于“折纸”思想的解码器的架构，二是创造性的引入弱标签监督来进行编码器初始化。
    从整个数据集平均重建指标上来看，用残差层改进解码器的方法使得基准模型获得了更稳定的学习能力以及对超参数调参的鲁棒性，缩小了与基于识别机制的模型的理论极限（Oracle-NN）之间的差距。
    ...
    \item[3.] 提出了对深度神经网络内在机制的丰富理解，并设计了实验流程进行了成功的验证。提出的新颖观点为：影响三维重建深度神经网络在
    识别机制与重建机制之间偏倚的主要因素为数据集的本质特征。并且基于计算机图形学算法创造性的创建了有不同聚类特征的形状插值数据集，并在此基础上
    进行了大规模的神经网络训练与测试，并利用自定义的度量指标以及数据集降维可视化分别定量与定性的验证了猜想。这一观点的提出与成功验证有望带给三维重建的研究者们关于该问题全新的认识，
    并启发更多与数据相关的模型方法。
\end{itemize}
