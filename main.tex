...
\end{figure}
根据论文PSGN~\cite{fan2017point}提出的基于“编码器-解码器”架构的深度神经网络模型，我们进行了如图\ref{fig:ae_baseline}的实现。网络的主要架构如下：
\begin{itemize}
...
对于非深度学习基准模型Oracle Nearest Neighbor, 我们展示了所有重建样本的损失平均值。
对于基于自编码器架构的深度学习基准模型，我们使用Adam优化器，训练了100个周期，使其达到收敛，并对超参数进行调参，总共进行了8次训练，最终展示了最优的
重建效果。如表\ref{table:metric_per_baseline}。

网络很容易陷入局部最优，而且可以看到损失收敛的很早，这说明网络的学习能力不强。这是因为我们的损失函数
是三维点云相关的，并不能显示的指导编码器来学习某种图像特征，我们尝试了用码字放入图片解码器中，发现并没有图片恢复出来，
我们也尝试将码字经过T-SNE可视化，发现没有良好的分类效果。因此需要好的初始化和更深的网络结构来提高收敛能力。
\begin{table}
    \centering
...
\begin{figure}[!htb]
        \centering
        \includegraphics[width=.40\textwidth]{figs/model/ae_trainingloss.png}
        \includegraphics[width=.40\textwidth]{figs/model/ae_testloss.png}
